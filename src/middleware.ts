...
export function middleware(request: NextRequest) {
  ...
  // 1. Check if the pathname already has other supported locale prefixes
  ...
  // 2. Detect target locale
  let locale = request.cookies.get('NEXT_LOCALE')?.value || defaultLocale;
  if (!locales.includes(locale)) locale = defaultLocale;

  // 3. For non-default locales (ru, en, zh), redirect to prefixed path if missing
  if (locale !== defaultLocale) {
    request.nextUrl.pathname = `/${locale}${pathname === '/' ? '' : pathname}`;
    return NextResponse.redirect(request.nextUrl);
  }

  // 4. For default locale (uz), rewrite internally to /[lang] path without visible prefix
