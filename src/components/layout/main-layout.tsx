...
const MainLayout: FC<Readonly<{ children: ReactNode }>> = ({ children }) => {
    ...
    useEffect(() => {
        setAnalyticsAllowed(hasCookieConsent());
        const allowAnalytics = () => setAnalyticsAllowed(true);
        window.addEventListener(COOKIE_CONSENT_EVENT, allowAnalytics);

        return () => window.removeEventListener(COOKIE_CONSENT_EVENT, allowAnalytics);
    }, []);
